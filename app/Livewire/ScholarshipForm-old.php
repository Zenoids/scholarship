...
class ScholarshipForm extends Component
{
    ...
    public function renewFormReset()
    {
        $this->reset(
            'course_id',
            'course_year',
    ...
    public function educationUpdate()
    {
        ...
        $student = StudentEducation::create([
    ...
    public function finalsubmit()
    {
        ...
        if ($this->photofile) {
            $image_file_path = $this->photofile->store('photos', 'public');
        }

        if ($this->adhaarfile) {
            $adhaar_file_path = $this->adhaarfile->store('adhaar', 'public');
        }

        if ($this->feesfile) {
            $fees_file_path = $this->feesfile->store('fees', 'public');
        }

        if ($this->marksfile) {
            $marks_file_path = $this->marksfile->store('marks', 'public');
        }
        if ($this->passbookfile) {
            $passbook_file_path = $this->passbookfile->store('passbook', 'public');
        }
    ...
    public function renewSubmission()
    {

        // dd($this->selectedDistrict);
        // dd($this->unit_admin_id);

        $renew_fees_file_path = null;
        $renew_marks_file_path = null;

        if ($this->renewfeesfile) {
            $renew_fees_file_path = $this->renewfeesfile->store('fees', 'public');
        }

        if ($this->renewmarksfile) {
            $renew_marks_file_path = $this->renewmarksfile->store('marks', 'public');
        }
        ...
        $studentRenewUpload = RenewedUpload::create([
            'renewfeesfile' => $renew_fees_file_path,
            'renewmarksfile' => $renew_marks_file_path,
            'studentId' => $studentId,
        ]);
...

    {
        ...
        $hasRenewalRegistered = Student::where('user_id', $user->id)->value('renewal');
        // dd($hasRenewalRegistered);
        $courses = Course::all();
        ...
        return view('livewire.scholarship-form', ['courses' => $courses, 'hasRegistered' => $hasRegistered, 'hasRenewalRegistered' => $hasRenewalRegistered, 'currentPage' => $currentPage, 'user' => $user]);
    }
